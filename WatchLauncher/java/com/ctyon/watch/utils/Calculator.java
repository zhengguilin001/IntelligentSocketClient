...
public class Calculator {
    ...
    public static void main(String[] args) {
        ...
        while (s.hasNextLine()) {
            ...
            if (result != null) {
                // 处理结果并打印
                System.out.println(
                        CaculatorUtils.formatResult(String.format("%." + CaculatorUtils.RESULT_DECIMAL_MAX_LENGTH + "f", result)));
            }
        }
    ...
    public Double prepareParam(String str) {
        ...
        // 长度校验
        if (str.length() > CaculatorUtils.FORMAT_MAX_LENGTH) {
        ...
        // 校验格式
        if (!CaculatorUtils.checkFormat(str)) {
            System.out.println("表达式错误！");
            return null;
        }
        // 处理表达式，改为标准表达式
        str = CaculatorUtils.change2StandardFormat(str);
        // 拆分字符和数字
    ...
    public Double doCalculate(String symStr, List<Double> numLst) {
        ...
        while (symStack.isEmpty() || !(symStack.getLast() == '=' && symStr.charAt(j) == '=')) {// 形如：
            ...
            if (CaculatorUtils.symLvMap.get(symStr.charAt(j)) > CaculatorUtils.symLvMap.get(symStack.getLast())) {// 比较符号优先级，若当前符号优先级大于前一个则压栈
                if (symStr.charAt(j) == '(') {
                ...
                switch (sym) {
                case '+':
                    numStack.add(CaculatorUtils.plus(num1, num2));
                    break;
                case '-':
                    numStack.add(CaculatorUtils.reduce(num1, num2));
                    break;
                case '*':
                    numStack.add(CaculatorUtils.multiply(num1, num2));
                    break;
                case '/':
                    if (num2 == 0) {// 除数为0
                    ...
                    numStack.add(CaculatorUtils.divide(num1, num2));
                    break;
                }
            }
        }
